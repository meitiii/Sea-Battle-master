...
void BattleshipGame::changetoCPUTurn()
{
    ...
    else if(*cpuplayer->hits==4){
        animeframes = randomnum(ANIMEFRAMES*2/4) + ANIMEFRAMES*2/4;

    }
    else if(*cpuplayer->hits==5){
        animeframes = randomnum(ANIMEFRAMES*3/4) + ANIMEFRAMES*3/4;

    }
    else if(*cpuplayer->hits==6){
        animeframes = randomnum(ANIMEFRAMES/4) + ANIMEFRAMES/4;

    }
    else if(*cpuplayer->hits==7){
        animeframes = randomnum(ANIMEFRAMES/2) + ANIMEFRAMES/2;

    }
    else if(*cpuplayer->hits==8){
        animeframes = randomnum(ANIMEFRAMES*3/4) + ANIMEFRAMES*3/4;

    }
    else if(*cpuplayer->hits==9){
        animeframes = randomnum(ANIMEFRAMES/4) + ANIMEFRAMES/4;

    }
    else if(*cpuplayer->hits==10){
        animeframes = randomnum(ANIMEFRAMES/2) + ANIMEFRAMES/2;
